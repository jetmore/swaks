...
=item Deliver a standard test email to user@example.com on port 25 of test-server.example.net:

swaks --to user@example.com --server test-server.example.net

=item Deliver a standard test email, requiring CRAM-MD5 authentication as user me@example.com.  An "X-Test" header will be added to the email body.  The authentication password will be prompted for.

swaks --to user@example.com --from me@example.com --auth CRAM-MD5 --auth-user me@example.com --header-X-Test "test email"

=item Test a virus scanner using EICAR in an attachment.  Don't show the message DATA part.:

swaks -t user@example.com --attach - --server test-server.example.com --suppress-data </path/to/eicar.txt

=item Test a spam scanner using GTUBE in the body of an email, routed via the MX records for example.com:

swaks --to user@example.com --body /path/to/gtube/file

=item Deliver a standard test email to user@example.com using the LMTP protocol via a UNIX domain socket file

swaks --to user@example.com --socket /var/lda.sock --protocol LMTP

=item Report all the recipients in a text file that are non-verifyiable on a test server:
...
=item --server [destination mail server[:port]], -s [destination mail server[:port]]
...
=item --local-interface [IP or hostname], -li [IP or hostname]
...
=item --to [email-address[,email-address,...]], -t [email-address[,email-address,...]]
...
=item --from [email-address], -f [email-address]
...
=item --helo [helo-string], --ehlo [helo-string], --lhlo [helo-string], -h [helo-string]
...
=item --quit-after [stop-point], -q [stop-point]
...
HELO = HELO, Implied Options: "-p 25"

=item SSMTP

HELO = EHLO->HELO, Implied Options: "-tlsc -p 465"

=item SSMTPA

HELO = EHLO->HELO, Implied Options: "-a -tlsc -p 465"

=item SMTPS

HELO = HELO, Implied Options: "-tlsc -p 465"

=item ESMTP

HELO = EHLO->HELO, Implied Options: "-p 25"

=item ESMTPA

HELO = EHLO->HELO, Implied Options: "-a -p 25"

=item ESMTPS

HELO = EHLO->HELO, Implied Options: "-tls -p 25"

=item ESMTPSA

HELO = EHLO->HELO, Implied Options: "-a -tls -p 25"

=item LMTP

HELO = LHLO, Implied Options: "-p 24"

=item LMTPA

HELO = LHLO, Implied Options: "-a -p 24"

=item LMTPS

HELO = LHLO, Implied Options: "-tls -p 24"

=item LMTPSA

HELO = LHLO, Implied Options: "-a -tls -p 24"
...
=item --auth [auth-type[,auth-type,...]], -a [auth-type[,auth-type,...]]
...
=item --auth-optional [auth-type[,auth-type,...]], -ao [auth-type[,auth-type,...]]
...
=item --auth-optional-strict [auth-type[,auth-type,...]], -aos [auth-type[,auth-type,...]]
...
=item --auth-user [username], -au [username]
...
=item --auth-password [password], -ap [password]
...
=item --auth-map [auth-alias=auth-type[,...]], -am [auth-alias=auth-type[,...]]
...
=item --data [data-portion], -d [data-portion]
...
=item --add-header [header], -ah [header]
