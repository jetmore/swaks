...
Specify which TCP port on the target is to be used, or prompt if no argument is listed.  The argument can be a service name (as retrieved by getservbyname(3)) or a port number.  The default port is smtp/25 unless infiuenced by the --protocol or --tls-on-connect options.
...
Used when searching for a .swaksrc configuration file.  See OPTION PROCESSING -> CONFIGUATION FILES above.
